...
def _pr_suite_evidence(gh, spec: dict, view: dict, checks: list, ref: str,
    ...
    if any(c.get("workflow") == spec["workflow"] for c in _passing_checks(checks)):
...
def _epic_suite_evidence(gh, branch: str, children: list, epic_files: list,
    ...
    suite), `unattested` (attestable suites still `missing`), `missing` (their workflows),
    ...
    chain = None
    suites, unattested, missing, breaks, carried = {}, [], [], {}, set()
    for spec in REQUIRED_WORKFLOWS:
        ...
        if status is None:
            ...
            if suite not in NON_ATTESTABLE_SUITES:
                unattested.append(suite)
        suites[suite] = status
    return {"suites": suites, "unattested": unattested, "missing": missing, "breaks": breaks,
